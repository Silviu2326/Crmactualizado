...
const CommandMessages: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    const viewResponse = handleViewCommand(currentMessage);
    const weekResponse = handleWeekCommand(currentMessage);
    const weekListResponse = handleWeekListCommand(currentMessage);
    const scrollResponse = handleScrollCommand(currentMessage);
    const addSessionResponse = handleAddSessionCommand(currentMessage);
    const response = viewResponse || weekResponse || weekListResponse || scrollResponse || addSessionResponse;
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const assistantMessage: Message = {
          ...
          text: response || '¡Gracias por tu mensaje! Estoy procesando tu solicitud.',
          sender: 'assistant',
          timestamp: new Date(),
          status: 'sent',
        };
