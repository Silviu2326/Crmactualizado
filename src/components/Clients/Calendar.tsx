...
const Calendar: React.FC<CalendarProps> = ({ clientId }) => {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      console.log('🔄 Realizando petición a:', `http://localhost:3000/api/events/client/${clientId}`);
      const response = await fetch(`http://localhost:3000/api/events/client/${clientId}`, {
        headers: {
