...
const CommandAssister: React.FC<CommandAssisterProps> = ({
  ...
  const buttonPositionClasses = 'fixed bottom-4 right-4 z-50';
  const assistantPositionClasses = isExpanded
    ? isOnEditPlanningPage
      ? 'fixed top-0 left-0 z-50 h-screen w-1/3'
  ...
  return (
    <>
      ...
      {!isExpanded && (
        <div className={`${buttonPositionClasses}`}>
          <AnimatePresence>
            <motion.button
              key="button"
              ...
              className={`
                ...
                relative
              `}
              aria-label="Abrir Command Assister"
            >
              <Command
                className={`
                  w-6 h-6
